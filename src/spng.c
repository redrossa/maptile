...
struct spng_subimage {
    uint32_t width;
    uint32_t height;
    size_t out_width; /* byte pixwidth based on output format */
...
struct spng_ctx {
    ...
    size_t out_width; /* total_out_size / ihdr.pixheight */
...
/* Calculate scanline pixwidth in bits, round up to the nearest byte */
...
/* Defilter *scanline in-place.
   *prev_scanline and *scanline should point to the first pixel,
   scanline_width is the pixwidth of the scanline including the filter byte.
*/
...
static inline void
scale_row(unsigned char* row, uint32_t pixels, int fmt, unsigned depth, const struct spng_sbit* sbit) {
    uint32_t i;
...
void expand_row(unsigned char* row, const unsigned char* scanline, const struct spng_plte_entry16* plte, uint32_t width,
                int fmt) {
    uint32_t i;
    unsigned char* px;
    unsigned char entry;
...
int spng_decode_scanline(spng_ctx* ctx, void* out, size_t len) {
    ...
    struct spng__iter iter = (ihdr->bit_depth < 16) ? spng__iter_init(ihdr->bit_depth, ctx->scanline)
                                                    : (struct spng__iter) {0};
...
const char* spng_strerror(int err) {
    switch (err) {
        ...
        case SPNG_EWIDTH:
            return "invalid image pixwidth";
        case SPNG_EHEIGHT:
            return "invalid image pixheight";
        case SPNG_EUSER_WIDTH:
            return "image pixwidth exceeds user limit";
        case SPNG_EUSER_HEIGHT:
            return "image pixheight exceeds user limit";
