...
namespace terrarium {
    ...
    int direct_tile_mflush(index_t itc, map* m, tile_transfer_t* tile_transf, va_list args);

    int decode_tile_fflush(index_t itc, map* m, tile_transfer_t* tile_transf, va_list args);

    int decode_tile_mflush(index_t itc, map* m, tile_transfer_t* tile_transf, va_list args);

    int map_fflush(index_t itc, map* m, tile_transfer_t* tile_transf, va_list args);

    int map_mflush(index_t itc, map* m, tile_transfer_t* tile_transf, va_list args);
}
